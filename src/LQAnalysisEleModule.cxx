 ...
 */
class LQAnalysisEleModule: public AnalysisModule {
public:
    
    explicit LQAnalysisEleModule(Context & ctx);
...
private:
  ...
  std::unique_ptr<Selection> njet_sel, fourjet_sel, TwoBTagL, BTagL, BTagM, BTagT, toptag, ntau_sel, ele_sel, muon_sel, onemuon_sel, twomuons_sel, isomuon_sel, leadingjet_sel, leadingjet300_sel, secondjet100_sel, elesamesign_sel, samesign_sel, met_sel, mbtau_sel;
...
LQAnalysisEleModule::LQAnalysisEleModule(Context & ctx){
    ...
    cout << "Hello World from LQAnalysisEleModule!" << endl;
    ...
    elesamesign_sel.reset(new EleTauSameSignCut());
...
bool LQAnalysisEleModule::process(Event & event) {
  ...
  //if(event.weight<0) return false;
    ...
    */
    ...
    if(elesamesign_sel->passes(event)) return false;
...
UHH2_REGISTER_ANALYSIS_MODULE(LQAnalysisEleModule)
