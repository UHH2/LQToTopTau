 ...
 */
class LQAnalysisTTbarSideBandPreModule: public AnalysisModule {
public:
    
    explicit LQAnalysisTTbarSideBandPreModule(Context & ctx);
...
private:
  ...
  std::unique_ptr<Selection> njet_sel, onejet_sel, bsel, ntau_sel, nmuon_sel;
...
LQAnalysisTTbarSideBandPreModule::LQAnalysisTTbarSideBandPreModule(Context & ctx){
    ...
    cout << "Hello World from LQAnalysisTTbarSideBandPreModule!" << endl;
    ...
    */
    ...
    onejet_sel.reset(new NJetCut(1,-1,20,5.0));
...
bool LQAnalysisTTbarSideBandPreModule::process(Event & event) {
  ...
  // fill hists before all selection cuts
    ...
    //if(!onejet_sel->passes(event)) return false;
...
UHH2_REGISTER_ANALYSIS_MODULE(LQAnalysisTTbarSideBandPreModule)
