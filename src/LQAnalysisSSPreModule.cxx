 ...
 */
class LQAnalysisSSPreModule: public AnalysisModule {
public:
    
    explicit LQAnalysisSSPreModule(Context & ctx);
...
private:
  ...
  std::unique_ptr<Selection> njet_sel, bsel, ntau_sel, nmuon_sel, samesign_sel, Mmumu_cut;
  ...
  std::unique_ptr<Hists> h_lq_ST400Only, h_tau_ST400Only, h_mu_ST400Only, h_ele_ST400Only, h_jet_ST400Only, h_event_ST400Only;
  std::unique_ptr<Hists> h_lq_Zcut, h_tau_Zcut, h_mu_Zcut, h_ele_Zcut, h_jet_Zcut, h_event_Zcut;
...
LQAnalysisSSPreModule::LQAnalysisSSPreModule(Context & ctx){
    ...
    cout << "Hello World from LQAnalysisSSPreModule!" << endl;
    ...
    */
    ...
    h_lq_ST400Only.reset(new LQAnalysisHists(ctx, "LQPreMod_LQ_ST400Only"));
    h_tau_ST400Only.reset(new TauHists(ctx, "LQPreMod_Taus_ST400Only"));
    h_mu_ST400Only.reset(new MuonHists(ctx, "LQPreMod_Muons_ST400Only"));
    h_ele_ST400Only.reset(new ElectronHists(ctx, "LQPreMod_Electrons_ST400Only"));
    h_jet_ST400Only.reset(new JetHists(ctx, "LQPreMod_Jets_ST400Only"));
    h_event_ST400Only.reset(new EventHists(ctx, "LQPreMod_Events_ST400Only"));

    h_lq_Zcut.reset(new LQAnalysisHists(ctx, "LQPreMod_LQ_Zcut"));
    h_tau_Zcut.reset(new TauHists(ctx, "LQPreMod_Taus_Zcut"));
    h_mu_Zcut.reset(new MuonHists(ctx, "LQPreMod_Muons_Zcut"));
    h_ele_Zcut.reset(new ElectronHists(ctx, "LQPreMod_Electrons_Zcut"));
    h_jet_Zcut.reset(new JetHists(ctx, "LQPreMod_Jets_Zcut"));
    h_event_Zcut.reset(new EventHists(ctx, "LQPreMod_Events_Zcut"));
...
bool LQAnalysisSSPreModule::process(Event & event) {
  ...
  // fill hists before all selection cuts
    ...
    h_lq_ST400Only->fill(event);
    h_tau_ST400Only->fill(event);
    h_mu_ST400Only->fill(event);
    h_ele_ST400Only->fill(event);
    h_jet_ST400Only->fill(event);
    h_event_ST400Only->fill(event);
    ...
    */
    ...
    h_lq_Zcut->fill(event);
    h_tau_Zcut->fill(event);
    h_mu_Zcut->fill(event);
    h_ele_Zcut->fill(event);
    h_jet_Zcut->fill(event);
    h_event_Zcut->fill(event);
...
UHH2_REGISTER_ANALYSIS_MODULE(LQAnalysisSSPreModule)
