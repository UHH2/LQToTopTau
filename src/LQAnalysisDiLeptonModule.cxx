 ...
 */
class LQAnalysisDiLeptonModule: public AnalysisModule {
public:
    
    explicit LQAnalysisDiLeptonModule(Context & ctx);
...
private:
  ...
  std::unique_ptr<Selection> njet_sel, TwoBTagL, BTagM, TwoBTagM, BTagT, ntau_sel, ele_sel, muon_sel, onemuon_sel, twomuons_sel, isomuon_sel, leadingjet_sel, leadingjet300_sel, secondjet100_sel, samesign_sel, met_sel, mbtau_sel,Mmumu_cut;
  ...
  std::unique_ptr<Hists> h_lq_TwoBTagM, h_tau_TwoBTagM, h_mu_TwoBTagM, h_ele_TwoBTagM, h_jet_TwoBTagM, h_event_TwoBTagM;
...
LQAnalysisDiLeptonModule::LQAnalysisDiLeptonModule(Context & ctx){
    ...
    cout << "Hello World from LQAnalysisDiLeptonModule!" << endl;
    ...
    h_ele_TwoBTagM.reset(new ElectronHists(ctx, "LQMod_Electrons_TwoBTagM"));
    h_mu_TwoBTagM.reset(new MuonHists(ctx, "LQMod_Muons_TwoBTagM"));
    h_jet_TwoBTagM.reset(new JetHists(ctx, "LQMod_Jets_TwoBTagM"));
    h_event_TwoBTagM.reset(new EventHists(ctx, "LQMod_Events_TwoBTagM"));
    h_tau_TwoBTagM.reset(new TauHists(ctx, "LQMod_Taus_TwoBTagM"));
    h_lq_TwoBTagM.reset(new LQAnalysisHists(ctx, "LQMod_LQ_TwoBTagM"));

 
}


bool LQAnalysisDiLeptonModule::process(Event & event) {
    ...
    //cout << "LQAnalysisDiLeptonModule: Starting to process event (runid, eventid) = (" << event.run << ", " << event.event << "); weight = " << event.weight << endl;
  ...
  */
    ...
    */
    ...
    h_lq_TwoBTagM->fill(event);
    h_tau_TwoBTagM->fill(event);
    h_mu_TwoBTagM->fill(event);
    h_ele_TwoBTagM->fill(event);
    h_jet_TwoBTagM->fill(event);
    h_event_TwoBTagM->fill(event);
...
UHH2_REGISTER_ANALYSIS_MODULE(LQAnalysisDiLeptonModule)
